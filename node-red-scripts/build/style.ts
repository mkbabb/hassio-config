...
export class StyleHelper {
  ...
  static panel(content: string[], title?: string): string {
    ...
    if (title) {
      const titlePadding = ' '.repeat(Math.max(0, (width - 4 - title.length) / 2));
      output += this.colors.muted('│ ') + titlePadding + this.colors.bold(title) + titlePadding + this.colors.muted(' │') + '\n';
      output += this.colors.muted('├' + '─'.repeat(width - 2) + '┤') + '\n';
    }
    
    content.forEach(line => {
      const padding = ' '.repeat(Math.max(0, width - 4 - this.stripAnsi(line).length));
      output += this.colors.muted('│ ') + line + padding + this.colors.muted(' │') + '\n';
    });
  ...
  static keyValue(key: string, value: string, keyColor = this.colors.muted, valueColor = this.colors.bold): string {
  ...
  static summary(stats: {
    ...
    if (stats.built !== undefined && stats.built > 0) {
      lines.push(`${this.colors.success('Built')}: ${this.colors.bold(stats.built.toString())}`);
    }
    
    if (stats.cached !== undefined && stats.cached > 0) {
      lines.push(`${this.colors.info('Cached')}: ${this.colors.bold(stats.cached.toString())}`);
    }
    
    if (stats.failed !== undefined && stats.failed > 0) {
      lines.push(`${this.colors.error('Failed')}: ${this.colors.bold(stats.failed.toString())}`);
    }
    
    if (stats.time !== undefined) {
      const timeStr = stats.time > 1000 ? `${(stats.time / 1000).toFixed(2)}s` : `${stats.time}ms`;
  ...
  static fileChanged(filename: string, action: 'added' | 'changed' | 'removed' = 'changed'): string {
    const actionColors = {
